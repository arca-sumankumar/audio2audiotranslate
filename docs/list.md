...
| [MODELS.md](MODELS.md) | **Models & backends, organized by pipeline stage.** Which model is active at each stage (STT / MT / TTS), what the product uses by default vs. evaluated candidates, real-model setup (`make models` / `make local-real`), the shipped gu/ml/mr ASR routing, the per-language medical ASR context, the MT-model catalogue (NLLB / Bergamot / IndicTrans2), and how each stage is evaluated with links to its eval doc. |
